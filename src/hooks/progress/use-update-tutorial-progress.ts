...
export function useUpdateTutorialProgress(
	ids: TutorialIdCollectionId
): UpdateProgressReturnInterface {
	...
	const { tutorialProgressLabel } = useTutorialProgress(ids)
	...
	// We don't want to run multiple mutations at once, even for quick changes
	 ...
	 */
	useEffect(() => {
		const { tutorialId, collectionId } = ids
		/**
		...
		if (needsInitialProgress || hasPositiveProgress) {
			 ...
			 * the mutation has settled with some result, we setIsRunningMutation(false).
			 */
			...
			const mutationArgs: TutorialProgressArgs = {
				...
				options: {
					onSettled: () => setIsRunningMutation(false),
				},
			}
			/**
