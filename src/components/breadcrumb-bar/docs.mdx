...
The `<BreadcrumbBar />` component is used to display a list of parent paths that lead to the current page.
...
On its own, this component cannot determine which breadcrumb links to render - it must be provided an array of `links` to render. To aid in generating these links, this component contains a `util` folder with helper functions.
...
const breadcrumbLinks = [
  /* Items can be manually added to allow breadcrumb links
     to the root of, and outside, the current basePath */
  { title: 'Waypoint', url: '/waypoint' },
  { title: 'Docs', url: `/waypoint/${basePath}` },
  ...getDocsBreadcrumbs({
    basePath: `waypoint/${basePath}` /* basePath, eg "docs", as typically named in page files */,
