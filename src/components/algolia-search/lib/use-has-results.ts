import { AutocompleteCollection, BaseItem } from '@algolia/autocomplete-core'
import { useState, useEffect } from 'react'

/**
 * If any collection has hit 'items', return true. This is used to help
 * determine whether the 'no results' state should show
 */
export function useHasResults(collections: AutocompleteCollection<BaseItem>[]) {
  const [hasResults, setHasResults] = useState(false)

  useEffect(() => {
    const resultsExist = getItemsExist(collections)
    if (resultsExist !== hasResults) {
      setHasResults(resultsExist)
    }
  }, [collections, hasResults])

  return hasResults
}

function getItemsExist(collections: AutocompleteCollection<BaseItem>[]) {
  let itemsExist = false
  for (const collection of collections) {
    if (collection.items.length > 0) {
      itemsExist = true
      break
    }
  }
  return itemsExist
}
