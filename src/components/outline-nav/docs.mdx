...
The `OutlineNavFromHeadings />` component provides an interface that is directly compatible with an array of `TableOfContentsHeading` items. This component has been constructed to ease the transition from the previously used `<TableOfContents />` component to `<OutlineNav />`. In the future, we may choose to reconsider how `headings` are generated - we could instead generate the `items` we need for `OutlineNav` server-side, which would remove the need for this compatibility layer component.
