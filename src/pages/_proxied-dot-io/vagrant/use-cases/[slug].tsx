	...
	return (
		<>
			...
			<IoUsecaseHero
				...
				pattern={require('/public/vagrant/img/usecase-hero-pattern.svg')}
			/>
			...
			<div className={s.callToAction}>
				<IoUsecaseCallToAction
					...
					pattern={require('/public/vagrant/img/usecase-callout-pattern.svg')}
				/>
			</div>
...
UseCasePage.layout = VagrantIoLayout

export async function getStaticPaths() {
	const query = proxiedRivetClient('nomad')
	const { allVagrantUseCases } = await query({
		query: useCasesQuery,
	})

	return {
		paths: allVagrantUseCases.map((page) => {
			return {
				params: {
					slug: page.slug,
				},
...
export async function getStaticProps({ params }) {
	...
	const page = allVagrantUseCases.find((page) => page.slug === slug)
