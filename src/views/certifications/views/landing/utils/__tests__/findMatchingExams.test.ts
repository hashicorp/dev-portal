...
describe('findMatchingExams', () => {
	...
	it('should throw an error from a desired exam being not found', () => {
		...
		expect(() => findMatchingExams(desiredExams, exams)).toThrowError(
			`Exam ID ${UNFOUND_EXAM.id} was not found. Please ensure that the id is found in any of the exams within exam-cards.json in src/content/certifications/exams.`,
		)
	})
