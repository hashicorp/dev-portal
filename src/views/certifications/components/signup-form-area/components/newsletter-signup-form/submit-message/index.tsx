...
const SubmitMessage = ({
	...
	const msgError =
		'We’re having trouble getting you subscribed. Try again or reach out to'

	return (
		<div
			className={classNames([s.submittedMsg, s[appearance]])}
			data-test-id={'submitMessage'}
		>
			<div className={classNames([s.iconWrapper, s[submitStatus]])}>
				{isSuccessful ? (
					<>
						<CheckCircleIcon />
						<p className={s.submittedMsgText}>{msgSuccess}</p>
					</>
				) : (
					<>
						<AlertTriangleIcon />
						<p className={s.submittedMsgText}>
							{msgError}{' '}
							<a
								className={s.submittedMsgTextLink}
								href="mailto:hello@hashicorp.com"
							>
								hello@hashicorp.com
							</a>
							.
