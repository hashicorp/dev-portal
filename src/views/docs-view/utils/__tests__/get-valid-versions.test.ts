...
describe('getValidVersions', () => {
	...
	it('finds additional versions via redirect', async () => {
			...
			.mockImplementation((url: URL) => {
				return Promise.resolve({
					json: async () => {
						...
						return { versiontrs: [] } // No versions found for this path
					},
				})
			})
		...
		expect(result).toEqual([
			{
				...versions[0],
				path: `/${productSlugForLoader}/${oldUrl}`,
			},
			{
				...versions[1],
				path: null,
	...
	it('only adds paths for versions that were discovered via redirect', async () => {
		...
		expect(result[0].path).toBe(`/${productSlugForLoader}/${oldUrl}`)
		expect(result[1].path).toBe(null)
	})
})
