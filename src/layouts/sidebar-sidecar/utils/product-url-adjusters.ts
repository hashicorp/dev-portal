...
function prefixSentinelDocsUrls(inputUrl: string): string {
  ...
  // We gather some information on what link routes we're dealing with.
  // Note that on the client-side, "components/docs-anchor" will modify
  // URLs that start with any of the "basePath" strings defined in
  // the "basePaths" array in "data/{product.json".
  // Any such URLs will be prefixed with the product name, in this case
  // "sentinel". However, unlike other product sites, links in the Sentinel
  // docs already contain this prefix. So, we need to remove the prefix
  // from any URLs that will later be modified based on "basePaths", while
  // retaining it for URLs that will not match "basePaths" (namely
  // "downloads" and "changelog").
  // TODO: could this be simplified by doing all URL manipulations here
  // TODO: at server time, rather than also prefixing some URLs on the
  // TODO: client-side using "components/docs-anchor"?
  // TODO: (May be useful longer term as well, eg if authors become
  // TODO: accustomed to the new URL structure on dev-dot, and want to write
  ...
  // Note that "docs" links will not be caught by the above check,
  // as on the sentinel site they are found under `/sentinel/*`, rather
  // than under `/sentinel/docs` as defined by our basePaths here in dev-dot.
  // So, we assume that if an internal link prefixed with `/sentinel`
  // it NOT one of the other two known links (downloads and changelog),
  // then it must be a docs link.
