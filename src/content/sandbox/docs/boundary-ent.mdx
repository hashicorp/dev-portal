...
You must set up a Boundary worker with access to the targets you want to connect to. You need at least one worker (an egress worker to connect to the target), but if you only configure a single worker then you may have to expose
...
1. Configure a worker.
   ...
   [a resolvable address for the Boundary Enterprise cluster](/boundary/docs/configuration/worker#initial_upstreams). If your cluster is listening on a port other than 9201, then change that value too.
