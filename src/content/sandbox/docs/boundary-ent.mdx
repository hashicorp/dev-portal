...
- Targets:
  ...
  - `northwind-db` (on ubuntu-target-tcp host)
  - `ubuntu-target-tcp` (on ubuntu-target-tcp host)
  - `ssh-recording-target-priya` (on ubuntu-target-tcp host)
...
Completing this exercise allows you to enter the raffle for a free pro exam voucher 
for Terraform or Vault.
...
1. After connecting to the target, take a screenshot or a picture of what you find.
   Show the instructor the picture to enter the raffle for a free certification exam voucher!
...
What are the requirements of these components? How must they be configured to create a
functional and secure Boundary deployment? How do the ingress and egress workers fuction in
a multi-hop sessions deployment, and how can this provide enhanced security for your network?

You can check the controller or worker configurations by examining `/etc/boundary.d/boundary.hcl` 
file on the those hosts.

Use the following documentatation to deepen your understanding:
